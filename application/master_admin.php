        ...
        echo "<script>
              setTimeout(function() {
                Swal.fire({
                  ...
                  window.location = 'index.php?view=admin';
                });
              }, 100);
            </script>";
                      }
...
<?php 
    ...
    if (isset($_POST['update'])){
      ...
      if (trim($_POST['b'])==''){
        mysql_query("UPDATE rb_users SET username = '$_POST[a]',
                                         ...
                                         email = '$_POST[d]',
                                         no_telpon = '$_POST[e]',
                                         jabatan = '$_POST[f]' where id_user='$_POST[id]'") or die(mysql_error());
      }else{
        mysql_query("UPDATE rb_users SET username = '$_POST[a]',
                                         password = '$passs',
                                         nama_lengkap = '$_POST[c]',
    ...
    if (isset($_POST['tambah'])){
      $data = md5($_POST['b']);
      $passs=hash("sha512",$data);
      mysql_query("INSERT INTO rb_users (username, password, nama_lengkap, email, no_telpon, jabatan, level, aktif) VALUES('$_POST[a]','$passs','$_POST[c]','$_POST[d]','$_POST[e]','$_POST[f]','superuser','Y')") or die(mysql_error());
      echo "<script>
            setTimeout(function() {
              Swal.fire({
                icon: 'success',
