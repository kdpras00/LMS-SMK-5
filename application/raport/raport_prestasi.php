...
if ($act==''){ 
    if (isset($_POST['simpan'])){
            if ($_POST['status']=='Update'){
              ...
              mysql_query("INSERT INTO rb_nilai_prestasi VALUES(NULL,'$get_tahun','$_POST[nisn]','$get_kelas','$_POST[a]','$_POST[b]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        echo "<script>document.location='index.php?view=prestasi&tahun=$get_tahun&kelas=$get_kelas#$_POST[nisn]';</script>";
    }

    if ($get_delete != ''){
        mysql_query("DELETE FROM rb_nilai_prestasi where id_nilai_prestasi='$get_delete'");
        echo "<script>document.location='index.php?view=prestasi&tahun=$get_tahun&kelas=$get_kelas#$get_nisn';</script>";
    }
?> 
            <div class="col-xs-12">  
              <div class="box">
                ...
                <?php 
                  ...
                  if (isset($tampil) && $tampil) {
                        ...
                        while($r=mysql_fetch_array($tampil)){
                          if ($get_edit != ''){
                              $e = mysql_fetch_array(mysql_query("SELECT * FROM rb_nilai_prestasi where id_nilai_prestasi='$get_edit'"));
                              $name = 'Update';
                          }else{
                              $name = 'Simpan';
                          }
                      ...
                      if ($get_nisn==$r['nisn']){   
                        echo "<form action='index.php?view=prestasi&tahun=$get_tahun&kelas=$get_kelas' method='POST'>
                                <tr><td>$no</td>
                                  <td>$r[nisn]</td>
                        ...
                        echo "<form action='index.php?view=prestasi&tahun=$get_tahun&kelas=$get_kelas' method='POST'>
                                ...
                                $pe = mysql_query("SELECT * FROM rb_nilai_prestasi where id_tahun_akademik='$get_tahun' AND nisn='$r[nisn]' AND kode_kelas='$get_kelas'");
                                while ($n = mysql_fetch_array($pe)){
                                    echo "<tr>
                                            ...
                                            <td>$n[keterangan]</td>
                                            <td width='90px' align=center><a href='index.php?view=prestasi&tahun=".$get_tahun."&kelas=".$get_kelas."&edit=".$n['id_nilai_prestasi']."&nisn=".$r['nisn']."#$r[nisn]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                            <a href='index.php?view=prestasi&tahun=".$get_tahun."&kelas=".$get_kelas."&delete=".$n['id_nilai_prestasi']."&nisn=".$r['nisn']."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                          </tr>";
                                }
                          $no++;
                          }
