...
if ($_GET['act']==''){ 
    if (isset($_POST['simpan'])){
            if ($_POST['status']=='Update'){
              ...
              mysql_query("INSERT INTO rb_nilai_prakerin VALUES('','$_GET[tahun]','$_POST[nisn]','$_GET[kelas]','$_POST[a]','$_POST[b]','$_POST[c]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        echo "<script>document.location='index.php?view=prakerin&tahun=$_GET[tahun]&kelas=$_GET[kelas]#$_POST[nisn]';</script>";
    }

    if (isset($_GET['delete'])){
        mysql_query("DELETE FROM rb_nilai_prakerin where id_nilai_extrakulikuler='$_GET[delete]'");
        echo "<script>document.location='index.php?view=prakerin&tahun=$_GET[tahun]&kelas=$_GET[kelas]#$_POST[nisn]';</script>";
    }
?> 
            <div class="col-xs-12">  
                ...
                <?php 
                  ...
                  if (isset($tampil)) {
                    ...
                    while($r=mysql_fetch_array($tampil)){
                      if (isset($_GET['edit'])){
                          $e = mysql_fetch_array(mysql_query("SELECT * FROM rb_nilai_prakerin where id_nilai_extrakulikuler='$_GET[edit]'"));
                          $name = 'Update';
                      }else{
                          $name = 'Simpan';
                      }
                  ...
                  if ($_GET['nisn']==$r['nisn']){   
                    echo "<form action='index.php?view=prakerin&tahun=$_GET[tahun]&kelas=$_GET[kelas]' method='POST'>
                            <tr><td>$no</td>
                              <td>$r[nisn]</td>
                    ...
                    echo "<form action='index.php?view=prakerin&tahun=$_GET[tahun]&kelas=$_GET[kelas]' method='POST'>
                            ...
                            $pe = mysql_query("SELECT * FROM rb_nilai_prakerin where id_tahun_akademik='$_GET[tahun]' AND nisn='$r[nisn]' AND kode_kelas='$_GET[kelas]'");
                            while ($n = mysql_fetch_array($pe)){
                                echo "<tr>
                                        ...
                                        <td>$n[deskripsi]</td>
                                        <td align=center><a href='index.php?view=prakerin&tahun=".$_GET['tahun']."&kelas=".$_GET['kelas']."&edit=".$n['id_nilai_extrakulikuler']."&nisn=".$r['nisn']."#$r[nisn]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                        <a href='index.php?view=prakerin&tahun=".$_GET['tahun']."&kelas=".$_GET['kelas']."&delete=".$n['id_nilai_extrakulikuler']."&nisn=".$r['nisn']."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                      </tr>";
                            }
                      $no++;
                      }
