       ...
       for ($i=1; $i<=$n; $i++){
         if (isset($_POST['jawab'.$i])){
           ...
           $kelas = $_POST['kelas'.$i];
            ...
            if ($cek['tot'] >= 1){
              mysql_query("UPDATE rb_pertanyaan_penilaian_jawab SET jawaban='$jawab' where id_pertanyaan_penilaian='$pertanyaan' AND nisn='$_SESSION[id]' AND kode_kelas='$kelas'") or die(mysql_error());
            }else{
              mysql_query("INSERT INTO rb_pertanyaan_penilaian_jawab VALUES('','$pertanyaan','$_SESSION[id]','$nisn','$jawab','teman','$kelas','".date('Y-m-d H:i:s')."')") or die(mysql_error());
          }
         }
       }
