...
$get_gettanggal = isset($_GET['gettanggal']) ? $_GET['gettanggal'] : '';

if ($act==''){ 
    if ($get_gettanggal != ''){
      $filtertgl = $get_gettanggal;
    }else{
      ...
      for ($i=1; $i <= $jml_data; $i++){
        ...
        if ($total >= 1){
           mysql_query("UPDATE rb_absensi_guru SET kode_kehadiran = '".$a[$i]."' where nip='".$nip[$i]."' AND kodejdwl='".$jdwl[$i]."' AND tanggal='$tglabsen'");
             ...
             mysql_query("INSERT INTO rb_absensi_guru VALUES (NULL,
                                                              '".$jdwl[$i]."',
                                                              '".$nip[$i]."',
                                                              '".$a[$i]."',
                                                              '$tglabsen',
        ...
        echo "<script>document.location='index.php?view=absenguru&tahun=".$get_tahun."&kelas=".$get_kelas."';</script>";
